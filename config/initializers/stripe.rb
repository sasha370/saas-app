Rails.configuration.stripe = {
    :publishable_key =>( ENV['STRIPE_TEST_PUBLISHABLE_KEY'] ||= Rails.application.credentials.stripe[:test_publishable_key].to_s),
    :secret_key => (ENV['STRIPE_TEST_SECRET_KEY'] ||= Rails.application.credentials.stripe[:test_secret_key].to_s)
...
Stripe.api_key =  (ENV['STRIPE_TEST_SECRET_KEY'] ||= Rails.application.credentials.stripe[:test_secret_key].to_s)
